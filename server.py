...
def find_adv():
    cursor = connection.cursor()
    langue=request.json['language']
    first_letter=request.json['first']
    last_letter=request.json['last']
    middle=request.json['middle']
    query = f"SELECT * FROM prolexeme_{langue} WHERE LABEL_PROLEXEME LIKE '{first_letter}%{middle}%{last_letter}'"
    cursor.execute(query)
    results = cursor.fetchall()
    cursor.close()
...
def topnotority():
    ...
    query = f"""
        ...
        LIMIT %s
    """
    cursor.execute(query, (year, type_val, limit))
    results = cursor.fetchall()
    cursor.close()
    response={'results':results}
    return jsonify(response)
...
def getprolexeme(token):
    ...
    table_name = f"prolexeme_{data['langue']}"
    cursor = connection.cursor()
    cursor.execute(f"select * from {table_name} where label_prolexeme= %s" ,(data['prolexeme'],)) 
    res = cursor.fetchall()
    cursor.close()
    if len(res)==0:
        exister = False
...
def getalias(token):
    ...
    cursor.execute(f"select * from prolexeme_{langue} , alias_{langue} where prolexeme_{langue}.num_pivot=alias_{langue}.num_pivot and prolexeme_{langue}.LABEL_PROLEXEME = %s" ,(prolexeme,))
    res = cursor.fetchall()
    return jsonify({'res':res})
...
def getderive(token):
    ...
    prolexeme=request.json['prolexeme']
    langue = request.json['langue']
    cursor.execute(f"select * from prolexeme_{langue} , derivative_{langue} where prolexeme_{langue}.num_pivot=derivative_{langue}.num_pivot and prolexeme_{langue}.LABEL_PROLEXEME = %s" ,(prolexeme,))
    res = cursor.fetchall()
    return jsonify({'res':res})
